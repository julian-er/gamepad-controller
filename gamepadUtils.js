...
export function isValidGamepad(gamepad) {
    return gamepad && 
           gamepad.buttons && 
           gamepad.axes && 
...
export function detectControllerType(gamepad) {
    ...
    if (CONTROLLER_MAPPINGS.nintendo.validation.idPatterns.some(pattern => pattern.test(id)) &&
...
export function applyDeadzone(value, deadzone = 0.1) {
    return Math.abs(value) > deadzone ? value : 0;
}

// Check if element is in viewport
export function isElementInViewport(element, rootMargin = 0) {
...
export function getFocusableElementsInViewport(containerSelector = null) {
...
export function calculateGridDimensions(elements, container) {
...
export function getElementDistance(el1, el2) {
...
export function findNearestInDirection(currentElement, allElements, direction) {
    const currentRect = currentElement.getBoundingClientRect();
    const currentX = currentRect.left + currentRect.width / 2;
    const currentY = currentRect.top + currentRect.height / 2;
    
    let candidates = allElements.filter(el => el !== currentElement);
    ...
    let nearestDistance = getElementDistance(currentElement, nearest);
...
export function ensureStatusElement(statusElementId) {
...
export function addNavigationStyles(options = {}) {
...
export function updateStatusElement(statusElementId, controllerType, isConnected) {
...
export function addGamepadDataAttributes(element, state) {
    if (!element) return;
...
export function removeGamepadDataAttributes(element) {
    if (!element) return;
