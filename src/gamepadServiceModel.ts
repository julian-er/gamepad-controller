...
// Temporary type for GamepadNavigationContext and GamepadContextManager
// Remove or replace with actual types when available
// @ts-ignore
...
export class GamepadService {
    ...
    onContextSwitch: ((newContext: any, oldContext: any) => void) | null;
    ...
    init() {
        ...
        if (this.options.autoCreateStatusElement && (this.options.statusElementId !== null && this.options.statusElementId !== undefined)) {
            ensureStatusElement(this.options.statusElementId ?? undefined);
        }
    ...
    handleGamepadConnected(event: GamepadEvent) {
        ...
        if (this.onControllerConnect) {
            this.onControllerConnect(gamepad);
        }
    }
    ...
    detectElements() {
        ...
        this.elements = getFocusableElementsInViewport(this.options.containerSelector ?? null as any);
        ...
        if (navMenuItems.length > 0) {
            ...
            const navElements = Array.from(navMenuItems).filter(el => 
                ...
                (!("disabled" in el) || !(el as any).disabled) &&
                !(el as HTMLElement).hasAttribute('disabled')
            );
            const otherElements = this.elements.filter(el => !(navElements as any).includes(el));
        ...
        this.gridDimensions = calculateGridDimensions(this.elements, container ?? document.body);
    ...
    handleShoulderNavigation(button: string) {
        if (this.options.enableDualContext) {
            // In dual context mode, use context manager
    ...
    gameLoop() {
        ...
        for (const gp of connectedGamepads) {
            if (gp && isValidGamepad(gp)) {
                ...
                const primaryButtonIndex = getPrimaryActionButtonIndex(this.currentControllerType as ControllerType);
                if (gp.buttons[primaryButtonIndex] && gp.buttons[primaryButtonIndex].pressed) {
                    if (currentTimestamp - this.lastButtonPress > (this.options.debounceTime ?? 0)) {
                ...
                if (Math.abs(leftStickX) > 0 || Math.abs(leftStickY) > 0) {
                    if (currentTimestamp - this.lastAxisMove > (this.options.debounceTime ?? 0)) {
                ...
                // D-pad navigation
                const dpadIndices = getDpadIndices(this.currentControllerType as ControllerType);
    ...
    getCurrentElement(): Element | null {
        if (this.options.enableDualContext) {
            ...
            return (this.elements && typeof this.focusedElementIndex === 'number' && this.elements[this.focusedElementIndex]) ? this.elements[this.focusedElementIndex] : null;
        }
    }
