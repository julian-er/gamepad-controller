import {
    ...
    updateGamepadStatus,
    handleScrolling,
} from '../navigation/NavigationEngine.js';
...
import { normalizeSnapshot } from '../input/normalizeSnapshot.js';
...
export function setupCustomEventListeners(
    ...
    const onConnected = (event: Event) => {
        ...
        const gamepad = normalizeSnapshot(customEvent.detail?.gamepad);
        if (!gamepad)
            return void state.onError?.(new Error('Invalid custom gamepad connect snapshot; event was ignored.'));
        handleGamepadConnected(createMockGamepadEvent(gamepad));
    };

    const onDisconnected = (event: Event) => {
        const customEvent = event as CustomEvent;
        const index = customEvent.detail?.gamepad?.index;
        if (!Number.isInteger(index) || index < 0)
            return void state.onError?.(new Error('Invalid custom gamepad disconnect snapshot; event was ignored.'));
        const known =
            state.gamepads[index] ??
            ({ index, id: String(customEvent.detail?.gamepad?.id ?? 'host-gamepad') } as Gamepad);
        state.pendingSnapshots = (state.pendingSnapshots ?? []).filter((snapshot) => snapshot.index !== index);
        handleGamepadDisconnected(createMockGamepadEvent(known));
    };
    ...
    const onStateChanged = (event: Event) => {
        ...
        if (customEvent.detail && customEvent.detail.gamepad) {
            const gamepad = normalizeSnapshot(customEvent.detail.gamepad);
            if (!gamepad) {
                state.onError?.(new Error('Invalid custom gamepad snapshot; state was ignored.'));
                return;
            }
            const wasConnected = !!state.gamepads[gamepad.index];
            state.gamepads[gamepad.index] = gamepad;
            (state.pendingSnapshots ??= []).push(gamepad);
            // A snapshot can be the first connection after host recovery.
            if (!wasConnected) handleGamepadConnected(createMockGamepadEvent(gamepad));
        }
    };
...
function getPadState(eventState: GamepadEventState, gp: Gamepad): PadInputState {
    ...
    if (!pad) {
        pad = {
            ...
            // Baseline the mapped primary button as well as generic button edges. A first
            // host/native snapshot describes current state, not a press that happened in us.
            lastPrimaryState:
                gp.buttons[getPrimaryActionButtonIndex(detectControllerType(gp) as ControllerType)]?.pressed ?? false,
        };
...
export function processGamepad(
    ...
    const generation = eventState.generation;
    const inputEpoch = eventState.inputEpoch;
    ...
    if (!eventState.isRunning || eventState.generation !== generation || eventState.inputEpoch !== inputEpoch) return;
    ...
    if (
        ...
        hadInput = true;
    if (!eventState.isRunning || eventState.generation !== generation) return;
    if (processRightStickScroll(gp, pad, navState, currentTimestamp, eventState)) hadInput = true;
...
export function processButtonEdges(gp: Gamepad, pad: PadInputState, eventState: GamepadEventState): boolean {
    ...
    for (let i = 0; i < gp.buttons.length; i++) {
        ...
        if (curr && !prev && typeof eventState.onButtonDown === 'function') {
            ...
            if (!eventState.isRunning || eventState.generation !== generation || eventState.inputEpoch !== inputEpoch)
                return hadInput;
        }
...
export function processPrimaryAction(
    ...
    const pressed = gp.buttons[primaryButtonIndex]?.pressed ?? false;
    const edge = pressed && !pad.lastPrimaryState;
    pad.lastPrimaryState = pressed;
    if (edge && navState.options.enableNavigation !== false) {
        const target: Element | null = navState.options.enableDualContext
            ? typeof (contextManager as { getActiveContext?: unknown } | undefined)?.getActiveContext === 'function'
                ? (contextManager?.getActiveContext()?.getCurrentElement() ?? null)
                : null
            : (navState.elements[navState.focusedElementIndex] ?? null);
        const completed = ActionDispatcher.dispatch(eventState, 'select', gp, target, () =>
            Boolean(
                navState.options.enableNavigation !== false &&
                (target || navState.options.enableDualContext) &&
                (!target || navState.isElementEligible?.(target) !== false) &&
                handleSelection(navState, contextManager, eventState.onNavigationRequest, target)
            )
        );
        if (completed) pad.lastButtonPress = currentTimestamp;
    }
}
...
export function processBackAction(
    ...
    if (backPressed && !pad.lastBackButtonState) {
        if (currentTimestamp - pad.lastBackTime > backCooldown) {
            if (navState.options.enableNavigation !== false)
                ActionDispatcher.dispatch(eventState, 'back', gp, null, () => {
                ...
                ActionDispatcher.dispatch(
                    ...
                    () =>
                        handleShoulderNavigation(
                            ...
                            button,
                            contextManager,
                            eventState.onNavigationMenuOpen,
                            eventState.onNavigationRequest
                        ),
                    undefined,
                    button
                );
...
export function processDirectionalMove(
    ...
    if (direction) {
        const effect = () => {
            if (navState.options.enableNavigation === false) return false;
            const before = navState.options.enableDualContext
                ? contextManager?.getActiveContext()?.getCurrentIndex()
                : navState.focusedElementIndex;
            handleNavigation(navState, direction, contextManager, updateFocusCallback);
            const after = navState.options.enableDualContext
                ? contextManager?.getActiveContext()?.getCurrentIndex()
                : navState.focusedElementIndex;
            return before !== after;
        };
...
export function processRightStickScroll(
    ...
    if (Math.abs(rightStickX) > 0 || Math.abs(rightStickY) > 0) {
        if (currentTimestamp - pad.lastScrollTime > (options.scrollDebounceTime ?? 50)) {
            ...
            if (
                options.enableNavigation !== false &&
                (eventState ? ActionDispatcher.dispatch(eventState, 'scroll', gp, null, effect) : effect())
            ) {
...
export function gameLoop(
    ...
    return function gameLoopImpl(): void {
        if (!eventState.isRunning || eventState.generation !== generation) {
        ...
        for (const candidate of connectedGamepads) {
            const gp = candidate && normalizeSnapshot(candidate);
        ...
        // A consumer callback may destroy the service. Do not leave a new frame owned by
        // this obsolete invocation after teardown.
        if (eventState.isRunning && eventState.generation === generation)
            eventState.animationFrameId = platform.requestAnimationFrame(gameLoopImpl);
    };
}
...
export function createCustomEventGameLoop(
    ...
    return function customEventGameLoopImpl(): void {
        ...
        // Host transports can deliver press+release before a rAF. Process every ordered
        // owned snapshot before continuing held input from the latest state.
        const pending = eventState.pendingSnapshots?.splice(0) ?? [];
        ...
        for (const gamepadIndex in eventState.gamepads) {
            ...
            if (gp && isValidGamepad(gp)) {
                ...
                if (
                    ...
                    eventState.generation !== generation ||
                    eventState.inputEpoch !== inputEpoch
                ) {
                    if (eventState.isRunning && eventState.generation === generation)
                        eventState.animationFrameId = platform.requestAnimationFrame(customEventGameLoopImpl);
                    return;
