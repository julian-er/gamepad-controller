...
import type { TypedEmitter } from '../events/EventEmitter.js';
import type { PlatformAdapter } from '../platform/PlatformAdapter.js';
import type { GamepadServiceEventMap } from '../service/GamepadService.js';
...
export class NavigationPolicy {
    ...
    requestBack(): void {
        ...
        if (this.platform.isBrowser) this.platform.historyBack();
    }
    ...
    requestNavigation(href: string, element: Element): void {
        ...
        if (this.platform.isBrowser) this.platform.assignLocation(href);
    }
}
