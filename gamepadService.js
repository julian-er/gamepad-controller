...
export function initGamepadNavigation(options = {}) {
...
export function gamepadService(containerSelector = null, options = {}) {
    const defaultOptions = {
...
export function initGamepadForPage(options = {}) {
    ...
    instance.onFocus = (element, index) => {
        console.log(`🎯 Focused: ${element.textContent || element.dataset.title || element.tagName}`);
    };

    instance.onSelect = (element, index) => {
        console.log(`✅ Selected: ${element.textContent || element.dataset.title || element.tagName}`);
        
        // Auto-handle navigation menu links
        if (element.classList.contains('nav-item') && element.href) {
            console.log(`🔗 Navigating to: ${element.href}`);
            // Navigation is already handled in the GamepadService
        }
    };

    instance.onControllerConnect = (gamepad, controllerType) => {
...
export function initDualContextGamepad(options = {}) {
    ...
    gamepadInstance.onFocus = (element, index) => {
        const context = gamepadInstance.getActiveContext();
        console.log(`🎯 Focused (${context?.id}): ${element.textContent || element.dataset.title || element.tagName}`);
    };

    gamepadInstance.onSelect = (element, index) => {
        const context = gamepadInstance.getActiveContext();
        console.log(`✅ Selected (${context?.id}): ${element.textContent || element.dataset.title || element.tagName}`);
    ...
    gamepadInstance.onControllerConnect = (gamepad, controllerType) => {
        console.log(`🎮 Controller connected: ${controllerType}`);
    };

    gamepadInstance.onControllerDisconnect = (gamepad) => {
        console.log('🎮 Controller disconnected');
    };
    ...
    gamepadInstance.onContextSwitch = (newContext, oldContext) => {
...
export const presets = {
    ...
    menu: (containerSelector = '.menu, .nav, nav, .nav-menu') => {
    ...
    grid: (containerSelector = '.grid, .gallery, .cards') => {
        return gamepadService(containerSelector, {
            navigationMode: 'grid',
    ...
    // Button navigation
    buttons: (containerSelector = '.buttons, .button-group') => {
        return gamepadService(containerSelector, {
            navigationMode: 'spatial',
    ...
    dualContext: (menuSelector = '.nav-menu, nav, .navigation', contentSelector = null) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (autoInit) {
        const preset = autoInit.dataset.gamepad;
        const container = autoInit.dataset.container;
